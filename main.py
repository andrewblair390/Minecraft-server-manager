...
class ServerManager(QObject):
    ...
    def start_server(self, max_ram="2G"):
        # Check Java
        ...
        self.log_signal.emit(f"Starting server with max RAM {max_ram}...")
        self.status_signal.emit("STARTING")
        self.current_status = "STARTING"

        try:
            self.process = subprocess.Popen(
                ["java", f"-Xmx{max_ram}", "-jar", self.jar_name, "nogui"],
                cwd=self.base_dir,
...
class Window(QWidget):
    def __init__(self):
        ...
        self.memory_input.setPlaceholderText("Max RAM (e.g., 2G)")
    ...
    def toggle_server(self):
        if self.server.is_ready() or self.server.is_running():
            ...
            max_ram = self.memory_input.text().strip() or "2G"
            self.server.start_server(max_ram=max_ram)
